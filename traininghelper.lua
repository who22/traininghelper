		...
		if autoCB[0] then
			lua_thread.create(function()
				...
				if cmd:find('/cb%s') then
					...
					sampSendDialogResponse(32700, 1, 18, nil)
...
function sampev.onSendChat(msg)
	if msg:find('стихи лестера') then
		sampAddChatMessage('----------------------------------------------------------------', 0xFFBB00)
		sampAddChatMessage('Ребята, не стоит вскрывать эту тему. Вы молодые, шутливые, вам все легко. Это не то.', 0xFFFFFF)
		sampAddChatMessage('Сюда лучше не лезть. Серьезно, любой из вас будет жалеть. Лучше закройте тему и забудьте об этом.', 0xFFFFFF)
		sampAddChatMessage('Я вполне понимаю, что данным сообщением вызову дополнительный интерес, но хочу...', 0xFFFFFF)
		sampAddChatMessage('сразу предостеречь пытливых — стоп. Остальных просто не найдут.', 0xFFFFFF)
		sampAddChatMessage('----------------------------------------------------------------', 0xFFBB00)
