...
function main()
	...
	if sampGetCurrentServerAddress() ~= '51.91.91.89' then 
		msgChat('Брат ты не на {90ce5a}TRAINING SERVER{FFFFFF}.... напиши /yes если хочешь выключить скрипт');
		sampRegisterChatCommand('yes', function(id)
			thisScript():unload()
		end)
	end


	msgChat('TrainingHelper 0.' .. scriptVersion .. ' by Gerald.myr запущен | /binds | /cmds | /menu')
